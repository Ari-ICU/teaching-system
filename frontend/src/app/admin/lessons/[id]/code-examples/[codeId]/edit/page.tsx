...
export default function EditCodeExamplePage({ params }: { params: Promise<{ id: string, codeId: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Code demo updated successfully!");
    ...
    <div className="page" style={{ maxWidth: '850px', margin: '0 auto' }}>
      ...
      <header className="page-header" style={{ marginBottom: '40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
           <Code size={24} color="var(--indigo)" />
           <span className="badge badge-indigo">EDIT DEMO</span>
        </div>
        <h1 className="page-title" style={{ fontSize: '32px' }}>Edit Code Demo</h1>
        <p className="page-subtitle">Update the source code or technical details for this demonstration.</p>
      </header>
